...
<template>
 ...
 <v-btn class="ma-2" id="q-2" color="primary" :to="{name: 'libroApa'}"  style="border-radius: 8px;  float: left; border: 1px solid green">
            ATRAS
          </v-btn>
</v-container>
  ...
  export default {
    ...
    methods: {
      ...
      editItem (item) {
        this.dialog = true
        console.log('item');
        this.axios.get(`/visualizarRef/${item}`)
        .then(res=>{
      ...
      this.close();
          this.axios.put(`/actualizar-registro/${item._id}`,item)
            .then(res =>{
        ...
        eliminarReferencias(id){
          ...
          this.axios.delete(`/eliminarRef/${id}`)
          .then(res=>{

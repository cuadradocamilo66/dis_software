...
<template>
 ...
 <v-btn class="ma-2" id="q-2" color="primary" :to="{name: 'articuloIso'}"  style="border-radius: 8px;  float: left; border: 1px solid green">
            ATRAS
          </v-btn>
</v-container>
  ...
  export default {
    ...
    methods: {
      ...
      editItem (item) {
        this.dialog = true
        console.log('item');
        this.axios.get(`/visualizarRef-Art-Iso/${item}`)
        .then(res=>{
      ...
      save () {
       ...
       ListarReferencias () {
        this.axios.get('/visualizar-Art-Iso')
         .then((response)=>{
      ...
      this.close();
          this.axios.put(`/actualizar-registro-Art-Iso/${item._id}`,item)
            .then(res =>{
              ...
              this.desserts[index].isbn=res.data.isbn;
            })
        ...
        eliminarReferencias(id){
          ...
          this.axios.delete(`/eliminarRef-Art-Iso/${id}`)
          .then(res=>{
